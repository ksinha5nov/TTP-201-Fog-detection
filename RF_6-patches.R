  ...
  # Load the data
  df <- read.csv("E:/TTP 201/Project/fog_aware_stats_horizontal_6_patches_pivot.csv")
  ...
  plot_confusion_matrix <- function(y_test, y_test_pred, model_name) {
    ...
    heatmap.2(as.matrix(cm), cellnote = labels, notecol = "black", density.info = "none", trace = "none", 
              ...
              main = paste("Confusion Matrix for", model_name, "6 patches (using Test Set)"), 
  ...
  plot(pr$curve[, 1], pr$curve[, 2], type = "l", col = "red", lwd = 2, main = "Precision-Recall Curve for Random Forest 6 patches", xlab = "Recall", ylab = "Precision")
  ...
  # Plot ROC curve
  plot(roc_obj, col = "darkorange", lwd = 2, main = sprintf("ROC curve 6 patches (AUC = %.2f)", roc_auc))
